<?php
/*
Template Name: KPMS - School Calendar
*/
?>
<?php
...
<!-- PAGE HERO -->
<section class="page-hero" style="background: linear-gradient(160deg, rgba(0,24,64,0.88) 0%, rgba(0,48,135,0.7) 50%, rgba(0,24,64,0.82) 100%), url('https://images.unsplash.com/photo-1506784983877-45594efa4cbe?w=1920&q=85') center/cover;">
  <div class="page-hero-label">Stay Informed</div>
  <h1 class="page-hero-title">School Calendar</h1>
  <p class="page-hero-subtitle">Important dates, events, and celebrations at KPMS for 2025–2026.</p>
  <div class="page-breadcrumb"><a href="/">Home</a> / <a href="#">Calendar</a> / Upcoming Events</div>
</section>

<div class="page-content">

  <?php
  // Dynamic events from Teacher Dashboard
  global $wpdb;
  $events_table = $wpdb->prefix . 'kpms_events';
  if ($wpdb->get_var("SHOW TABLES LIKE '$events_table'") === $events_table) {
      $today = current_time('Y-m-d');
      $dynamic_events = $wpdb->get_results($wpdb->prepare(
          "SELECT * FROM $events_table WHERE event_date >= %s ORDER BY event_date ASC, event_time ASC", $today
      ));
      if (!empty($dynamic_events)):
          $current_month = '';
  ?>
  <?php foreach ($dynamic_events as $evt):
      $evt_month = date('F Y', strtotime($evt->event_date));
      if ($evt_month !== $current_month):
          if ($current_month !== '') echo '</div>'; // close previous calendar-month
          $current_month = $evt_month;
  ?>
  <div class="calendar-month">
    <div class="calendar-month-title">📅 <?php echo esc_html($evt_month); ?></div>
  <?php endif; ?>
    <div class="cal-event">
      <div class="cal-event-date">
        <div class="dow"><?php echo esc_html(date('D', strtotime($evt->event_date))); ?></div>
        <div class="day"><?php echo esc_html(date('j', strtotime($evt->event_date))); ?></div>
      </div>
      <div class="cal-event-info">
        <h4><?php echo esc_html($evt->event_name); ?></h4>
        <p>
          <?php if (!empty($evt->event_time)): ?>🕐 <?php echo esc_html($evt->event_time); ?><?php endif; ?>
          <?php if (!empty($evt->location)): ?> · <?php echo esc_html($evt->location); ?><?php endif; ?>
          <?php if (!empty($evt->description)): ?><br><?php echo esc_html($evt->description); ?><?php endif; ?>
        </p>
      </div>
    </div>
  <?php endforeach; ?>
  <?php if ($current_month !== '') echo '</div>'; // close last calendar-month ?>

  <div style="text-align:center; margin:40px 0 50px; padding:20px; background:var(--ice); border-radius:12px;">
    <p style="font-size:14px; color:var(--text-muted); margin:0;">Events above are managed by KPMS staff. Additional scheduled events below.</p>
  </div>
  <?php
      endif;
  }
  ...
  <!-- March 2026 -->
  <div class="calendar-month">
    <div class="calendar-month-title">📅 March 2026</div>
    ...
    <div class="cal-event">
      ...
      <div class="cal-event-info">
        <h4>Pakistan Day Celebration Rehearsals</h4>
        <p>🕐 9:00 AM – 12:00 PM · All grades participate in flag ceremony and national anthem practice.</p>
      </div>
    </div>
    ...
    <div class="cal-event">
      ...
      <div class="cal-event-info">
        <h4>Spring Art Exhibition</h4>
        <p>🕐 10:00 AM – 2:00 PM · Student artwork showcase open to parents and community. Refreshments provided.</p>
      </div>
    </div>
    ...
    <div class="cal-event">
      ...
      <div class="cal-event-info">
        ...
        <p>🕐 1:00 PM – 5:00 PM · Individual meetings with teachers. Sign up for your time slot via the Parent Portal.</p>
      </div>
    </div>

    <div class="cal-event" style="border-left-color: var(--gold);">
      <div class="cal-event-date" style="background: var(--gold);">
        <div class="dow" style="color: var(--navy-deep);">Mon</div>
        <div class="day" style="color: var(--navy-deep);">23</div>
      </div>
      <div class="cal-event-info">
        <h4>🇵🇰 Pakistan Day — School Holiday</h4>
        <p>No classes. KPMS celebrates Pakistan Day. Students may attend the district ceremony with families.</p>
      </div>
    </div>
  </div>
  ...
  <div class="calendar-month">
    ...
    <div class="cal-event">
      ...
      <div class="cal-event-info">
        <h4>Open House for New Families</h4>
        <p>🕐 9:00 AM – 12:00 PM · Tour campus and meet our teachers — all are welcome. No appointment needed.</p>
      </div>
    </div>
    ...
    <div class="cal-event">
      ...
      <div class="cal-event-info">
        <h4>Science Fair</h4>
        <p>🕐 10:00 AM – 1:00 PM · Grades 3–5 present science projects. Parents welcome as judges!</p>
      </div>
    </div>
    ...
    <div class="cal-event" style="border-left-color: var(--gold);">
      <div class="cal-event-date" style="background: var(--gold);">
        <div class="dow" style="color: var(--navy-deep);">Mon</div>
        <div class="day" style="color: var(--navy-deep);">13</div>
      </div>
      <div class="cal-event-info">
        <h4>🌙 Ramadan Break Begins</h4>
        <p>School closed for Ramadan holidays. Classes resume based on Eid announcement. Check portal for updates.</p>
      </div>
    </div>
    ...
    <div class="cal-event">
      ...
      <div class="cal-event-info">
        <h4>Classes Resume After Eid</h4>
        <p>🕐 8:00 AM · Welcome back! Half day for settling in. Eid celebrations and sharing stories.</p>
      </div>
    </div>
  </div>
  ...
  <div class="calendar-month">
    ...
    <div class="cal-event">
      ...
      <div class="cal-event-info">
        ...
        <p>🕐 8:30 AM – 1:00 PM · Track events, relay races, and team games. Parents cheering encouraged!</p>
      </div>
    </div>
    ...
    <div class="cal-event">
      ...
      <div class="cal-event-info">
        <h4>Final Exams Begin (Grades 3–5)</h4>
        <p>Annual examinations begin. Exam schedule available on the Parent Portal and school noticeboard.</p>
      </div>
    </div>
    ...
    <div class="cal-event">
      ...
      <div class="cal-event-info">
        <h4>🎓 Annual Prize Distribution & Closing Ceremony</h4>
        <p>🕐 10:00 AM · Awards, performances, and farewell to graduating Grade 5 students. All families invited.</p>
      </div>
    </div>
  </div>

  <div style="text-align:center; margin-top:40px; padding:32px; background:var(--ice); border-radius:16px;">
    <h3 style="margin-top:0;">📥 Download Calendar</h3>
    <p>Download the full academic calendar as a PDF to keep on your fridge!</p>
    <a href="#" class="btn btn-primary" style="display:inline-block; padding:12px 28px; background:var(--blue); color:white; border-radius:10px; text-decoration:none; font-weight:700; font-size:14px;">Download 2025–26 Calendar (PDF)</a>
  </div>
